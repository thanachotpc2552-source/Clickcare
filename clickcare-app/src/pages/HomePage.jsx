...
เบื้องต้นแนะนำให้ปรึกษา **แพทย์อายุรกรรมทั่วไป (General Practice)** เพื่อประเมินอาการครับ`
  ...
  const sendAiMessage = async () => {
    ...
    try {
      // Call the backend AI Triage endpoint
      const res = await authFetch('/api/ai/triage', {
        method: 'POST',
        body: JSON.stringify({ symptom: userMsg })
      })

      if (!res.success) throw new Error(res.error || 'Triage failed')
      const data = res.data

      // Format response text nicely
      const levelTh = data.severity === 'High' ? '🔴 เสี่ยงสูง (ควรรีบพบแพทย์)' : data.severity === 'Medium' ? '🟡 เสี่ยงปานกลาง' : '🟢 เสี่ยงต่ำ';
      const formattedReply = `จากการวิเคราะห์อาการของท่าน:
- อาการที่น่าจะเป็น: **${data.conditionTh}** (${data.conditionEn})
- ระดับความรุนแรง: **${levelTh}**
- แนะนำพบแพทย์แผนก: **${data.department}**

💡 คำแนะนำเบื้องต้น:
${data.adviceTh}

*คำแนะนำนี้เป็นเพียงการวิเคราะห์เบื้องต้นเท่านั้น ไม่ทดแทนการตรวจจริงจากแพทย์*`;

      setAiChat(c => [...c, { role: 'ai', text: formattedReply }])

      // Auto recommend matching doctors
      if (data.recommendedDoctors && data.recommendedDoctors.length > 0) {
        setAiRecommendedDoctors(data.recommendedDoctors.slice(0, 3))
      } else if (data.department) {
        const matchingDocs = doctors.filter(d => 
          d.department.toLowerCase().includes(data.department.toLowerCase())
        ).slice(0, 3)
        if (matchingDocs.length > 0) setAiRecommendedDoctors(matchingDocs)
      }
    } catch (err) {
      console.warn('Backend Triage API failed, falling back to NLP', err)
